...
function json_orderer {
    local CP=$(one_line_pem $2)
    sed -e "s/\${CAPORT}/$1/" \
        -e "s#\${CAPEM}#$CP#" \
        organizations/orderer-template.json
}
...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/org1.example.com/connection-peer0-org1.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/org1.example.com/connection-peer0-org1.yaml
...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/org2.example.com/connection-peer0-org2.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/org2.example.com/connection-peer0-org2.yaml

CAPORT=9054
CAPEM=organizations/fabric-ca/ordererOrg/ca-cert.pem

echo "$(json_orderer $CAPORT $CAPEM)" > organizations/ordererOrganizations/example.com/connection-orderer.json
